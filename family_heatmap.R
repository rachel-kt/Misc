cluster_matrix_hm = read.csv("phosphatase_family_heatmap.csv", stringsAsFactors = F, row.names = 1)
...
hm = Heatmap(matrix_norm, split = cluster_matrix_merged$family, 
        cluster_rows = T, name = " ", 
        row_names_gp = gpar(fontsize = 5), show_column_dend = F,
        show_row_names = T,
        ...
        show_row_dend = F,
        show_column_names = F,
        left_annotation = rowAnnotation(pt = anno_empty(border = F, width = unit(25,"mm")))
        )
